...
class MLP(nn.Module):
    def __init__(
        self, input_size, output_size, hidden_layer_sizes, activation_name, p_dropout
    ):
...
class MLPEstimator:
    ...
    def fit(self, X_train, y_train, X_valid, y_valid, X_unlabeled=None):
        ...
        if self.verbose:
            epoch_train = tqdm(
                range(self.epochs), desc="Supervised training ; epochs", unit="epoch"
            )
        else:
        ...
        for _ in epoch_train:
            ...
            for i, (batchX, batchY) in enumerate(
                self.next_batch(X_train, y_train, self.batch_size)
            ):
            ...
            if self.early_stopping:
                ...
                batchY_valid = (
                    torch.from_numpy(y_valid).float().unsqueeze(1).to(self.device)
                )
        ...
        if X_unlabeled is not None:
            ...
            if self.verbose:
                epoch_train_unlabeled = tqdm(
                    range(self.epochs),
                    desc="Semi-supervised training ; epochs",
                    unit="epoch",
                )
            else:
            ...
            for _ in epoch_train_unlabeled:
                ...
                self.model.train(True)
                for i, (batchX, batchY) in enumerate(
                    self.next_batch(X_combined, y_combined, self.batch_size)
                ):
                    batchX = batchX.to(self.device)
                    batchY = batchY.to(self.device)
                    batchY.requires_grad = True
...
class MLPBinaryClassifier(nn.Module):
    ...
    def randomized_search(
        self, param_randomized, n_iter, cv=5, scoring="accuracy", verbose=3, n_jobs=1
    ):
        cv = StratifiedKFold(n_splits=cv, shuffle=True, random_state=1)
        ...
        print(
            f"Best hyperparameters randomized search : {randomized_search.best_params_}"
        )
        return results_df

    def fit(self):
        self.model.fit(
            self.X_train_standard,
            self.y_train_standard,
            self.X_valid_standard,
            self.y_valid_standard,
            self.X_unlabeled_standard,
        )
    ...
    def model_performance(self, metric="all"):
        ...
        scores_train = MLPBinaryClassifier.compute_metrics(
            metric, self.y_train, y_pred_train
        )
        y_pred_test = self.forward(self.X_test)
        scores_test = MLPBinaryClassifier.compute_metrics(
            metric, self.y_test, y_pred_test
        )
        data = {}
    ...
    def compute_integrated_gradients(self, X, baseline=None, steps=50):
        ...
        attributions = integrated_gradients.attribute(
            input_tensor, baseline, target=0, n_steps=steps
        )
        if self.model.device == "cuda":
            attributions = attributions.cpu()
        attributions_df = pd.DataFrame(
            attributions.detach().numpy().astype(float), columns=features
        )
        ...
        def custom_minmax_scaler(data, feature_range=(0, 100)):
            ...
            if max_val - min_val == 0:
                return (
                    ...
                    else np.full_like(data, feature_range[0])
                )
        ...
        normalized_data = custom_minmax_scaler(
            avg_abs_attributions.values.reshape(-1, 1)
        ).astype(float)
        np.set_printoptions(suppress=True, precision=2)
        normalized_attributions = pd.DataFrame(
            normalized_data, columns=["attribution"], index=features
        )
        sorted_attributions = normalized_attributions.sort_values(
...
map_output = {0: "Fully Paid", 1: "Charged Off"}
